...
def train_random_forest(features_df, target):
    ...
    Returns:
        ...
        best_params: the best tuned parameters consisting of the following: 
            n_estimators: Optimized between 20 & 200 
            max_depth: Optimized between 1 & 60  
            min_samples_split: Optimized between 2 & 20 
            min_samples_leaf: Optimized between 2 & 20  
    '''
    # Initialize optimization libray and define optimum variable values
    study = optuna.create_study(direction='maximize')
    study.optimize(lambda trial: optimize_random_forest(trial, features_df, target), n_trials=200)
    ...
    # Preprocessing number based features using mean based strategy
    ...
    pipeline = Pipeline([ ('preprocessor', preprocessor),
        ('rf', RandomForestClassifier(n_estimators=best_params['n_estimators'], max_depth=best_params['max_depth'],min_samples_split=best_params['min_samples_split'],min_samples_leaf=best_params['min_samples_leaf'], random_state=0))
    ])
